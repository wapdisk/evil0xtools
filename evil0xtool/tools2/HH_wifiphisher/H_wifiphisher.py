...
def target_APs():
    global APs, count
    os.system('clear')
    print '['+G+'+'+W+'] 要伪装下列AP请在任意时候按下Ctrl-C中断扫描'
    print '序号  频道   ESSID'
    print '---------------'
    for ap in APs:
...
def get_strongest_iface(exceptions=[]):
    ...
    for i in interfaces:
        ...
        print '['+G+'+'+W+'] 网络被发现通过网卡 '+G+i+W+': '+T+str(count)+W
...
def start_mon_mode(interface):
    print '['+G+'+'+W+'] 正在启动监控模式关闭 '+G+interface+W
    try:
...
if __name__ == "__main__":
    ...
    print '[' + T + '*' + W + '] 正在启动HTTP服务器于端口 ' + str(PORT)
    ...
    print '[' + T + '*' + W + '] 正在启动HTTPS服务器于端口 ' + str(SSL_PORT)
    ...
    print '[' + T + '*' + W + '] 租约已清除, DHCP已启动, 设置iptables'
